...
function SolveQuadraticProgram!(vX :: Vector{T}, vZ :: Vector{T}, mA, vB, mC, vD, mE, vF;
    ...
    for ii in 1:numIterations
        ...
        if (mod(ii, numItrConv) == 0)
            convFlag = CheckConvergence(vX, mA, vB, mC, vD, mE, vF, vS, vY, vZ, ϵAbs, ϵRel)
        end
    end
...
function CheckConvergence(vX :: Vector{T}, mA, vB, mC, vD, mE, vF, vS, vY, vZ, ϵAbs, ϵRel) where {T <: AbstractFloat}
